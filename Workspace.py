...
def Bokeh_Decision_Boundaries_2D(X, Variable1, Variable2, y, Estimators = [LDA,QDA,KNN,GNB,TREE], Notebook_Url = "None", Test_Size = 0.3, Random_State = None, Scale = True , Palette = "RdBu", Delta = 0.02, Output = 1):
    ...
    def modify_doc(doc):
        ...
        def update_plots(new):
            ...
            layout = gridplot([widgetbox(drop1, drop2, drop3, estimator_select,update_drop)],[row(plot) for plot in plots])
            return layout
        ...
        estimator_names = [str(estimator()).split("(")[0] for estimator in Estimators]
        estimator_indices = [str(idx) for idx , name in enumerate(estimator_names)]
        estimator_select = MultiSelect(title= "Estimators", options = estimator_indices, value = estimator_names)
        
        update_drop = Dropdown(label = "Update" ,default_value = "-", menu = [("Update", "Update")], value = "-")
    ...
    handle0 = show(app, notebook_url=Notebook_Url, notebook_handle = True)
